...
export function CommandPalette() {
  ...
  const groupLabels: Record<SearchItemType, string> = {
    booking: t('groupBookings'),
    exporter: t('groupExporters'),
    naviera: t('groupNavieras'),
    alert: t('groupAlerts'),
    page: t('groupPages'),
  };

  const grouped = useMemo(
    () => GROUP_ORDER.reduce<Record<SearchItemType, SearchItem[]>>(
      (acc, type) => {
                ...
                return GROUP_ORDER.map((type) => {
                  ...
                  return (
                    <Command.Group
                      ...
                      heading={
                        <span className={cn(
                          'block px-4 py-1 font-mono text-[10px] uppercase tracking-widest text-ink-4',
                          !isFirst && 'border-t border-[var(--line-soft)] mt-1',
                        )}>
                          {groupLabels[type]}
                        </span>
                      }
