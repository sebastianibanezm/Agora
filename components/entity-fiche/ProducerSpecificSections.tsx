...
export async function ProducerSpecificSections({ producer }: Props) {
  ...
  return (
    <>
      ...
      {/* Certified products */}
      <section>
        <h2 style={{ fontSize: '15px', fontWeight: 600, color: '#e2e8f0', marginBottom: '12px' }}>{t('certifiedProducts')}</h2>
            ...
            <div key={cp.productId} style={{ background: '#1a1f2e', border: '1px solid #ffffff12', borderRadius: '8px', padding: '14px', display: 'flex', flexDirection: 'column', gap: '8px' }}>
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' }}>
                ...
                <span style={{ fontSize: '11px', color: cp.requiresColdChain ? '#7DD3FC' : '#64748b' }}>
                  {cp.requiresColdChain ? t('coldChain') : t('ambient')}
                </span>
              </div>
              <div>
                <div style={{ fontSize: '10px', color: '#64748b', marginBottom: '4px' }}>{t('season')}: {cp.seasonStart}–{cp.seasonEnd}</div>
                <MiniSeasonBar start={cp.seasonStart} end={cp.seasonEnd} />
              </div>
              {cp.coldProtocol && (
                <div style={{ fontSize: '11px', color: '#94a3b8' }}>{t('protocol')}: {cp.coldProtocol}</div>
              )}
      ...
      {/* SAG certifications */}
      <section>
        <h2 style={{ fontSize: '15px', fontWeight: 600, color: '#e2e8f0', marginBottom: '12px' }}>{t('sagCertifications')}</h2>
